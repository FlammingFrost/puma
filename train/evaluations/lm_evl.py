...
from retrieval.embedder import LM

# Load linear model and evaluate it on test query embeddings

def load_model(model_path, input_dim, output_dim, device):
    model = LM(input_dim=input_dim, output_dim=output_dim)
...
def main(args):
    ...
    model = load_model(args.model_path, input_dim, output_dim, args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cosine_similarities_path", type=str, help="Path to save the cosine similarities", default="models/embeddings/small_baseline_cosine_similarities.npy")
