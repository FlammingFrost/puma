...
def eval():
    ...
    model_4bit = AutoModel.from_pretrained(MODEL_NAME, load_in_4bit=True, device_map="auto")
    model_4bit.eval()
    ...
    try:
        with open("quantization/eval_results_4bit.txt", "a") as f:
...
def test():
    ...
    try:
        with open("quantization/test_results_4bit.txt", "a") as f:
            f.write(f"Top1 Recall: {top1_recall:.4f}\n")
            f.write(f"Top5 Recall: {top5_recall:.4f}\n")
    except Exception as e:
        print(f"Error writing to file: {e}")
