...
def query_codebase(path: str, queries: list, **kwargs) -> str:
    ...
    codebase_prompt = ingest(path, **kwargs)
    ...
    Combined Queries: {combined_query}
    
    Codebase:
    {codebase_prompt}
    
    Provide concise and informative responses in the following format:
    [Question-{{index}}]:\n{{Question}}
    [Answer-{{index}}]:\n{{Answer}}
    [Explain-{{index}}]:\n{{Explain and possible suggestion on edition}}
    """
